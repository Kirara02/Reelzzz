...
export const LikesSlice = createSlice({
  ...
  reducers: {
    ...
    addLikedComment: (state, action: PayloadAction<LikedComment>) => {
      const index = state.LikedComment.findIndex(user => user.id === action.payload.id);
      if (index !== -1) {
    ...
    addLikedReply: (state, action: PayloadAction<LikedReply>) => {
      const index = state.LikedReply.findIndex(user => user.id === action.payload.id);
      if (index !== -1) {
...
export const {addLikeReel, addLikedComment, addLikedReply} = LikesSlice.actions;
