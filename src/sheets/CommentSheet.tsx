...
const CommentSheet = (props: SheetProps<'comment-sheet'>) => {
  ...
  const handleReplyComment = async (data: any) => {
    ...
    const res = await dispatch(postReply(commentPostData, props.payload?.commentsCount || 0));

    if (res) {
      const tempCommentIndex = commentData.findIndex(comment => comment._id === replyCommentId);

      // Replace the temporary comment with the actual comment
      if (tempCommentIndex !== -1) {
        commentData[tempCommentIndex].repliesCount = commentData[tempCommentIndex].repliesCount + 1;
  ...
  return (
    <ActionSheet
      ...
      animated>
      <CustomText variant="h7" fontFamily={FONTS.SemiBold} style={styles.header}>
        Comments
      </CustomText>
