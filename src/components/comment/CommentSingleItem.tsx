...
const CommentSingleItem: React.FC<CommentSingleItemProps> = ({
  ...
  const commentMeta = useMemo(() => {
    return {
      isLiked:
        likedComment?.find((ritem: any) => ritem.id === comment._id)?.isLiked ?? comment?.isLiked,
      likesCount:
        likedComment?.find((ritem: any) => ritem.id === comment._id)?.likesCount ??
        comment.likesCount,
    };
  ...
  const replyMeta = useMemo(() => {
    return {
      isLiked:
        LikedReply?.find((ritem: any) => ritem.id === comment._id)?.isLiked ?? comment.isLiked,
      likesCount:
        LikedReply?.find((ritem: any) => ritem.id === comment._id)?.likesCount ??
        comment.likesCount,
    };
  ...
  const likeComment = async () => {
    ...
    if (!isReply) {
      await dispatch(
        toggleLikeComment(comment._id, commentMeta?.likesCount, commentMeta?.isLiked ?? false),
      );
      return;
    }
    await dispatch(
      toggleLikeReply(comment._id, replyMeta?.likesCount, replyMeta?.isLiked ?? false),
    );
  };
  ...
  return (
    <GestureDetector gesture={Gesture.Exclusive(doubleTap, longPress)}>
      <Animated.View
        style={[styles.commentContainer, {backgroundColor: backgroundColorInterpolate}]}>
        <TouchableOpacity
          onPress={() => {
        ...
        <View style={styles.textContainer}>
          <View style={styles.flexRow}>
            <CustomText fontFamily={FONTS.SemiBold} variant="h9" style={styles.username}>
              {comment?.user?.username}
            </CustomText>
            <CustomText variant="h9" fontFamily={FONTS.Medium} style={styles.timestamp}>
            ...
            {comment?.user?._id == user?._id && (
              <CustomText variant="h9" style={styles.timestamp} fontFamily={FONTS.Medium}>
                • Author{' '}
              </CustomText>
            )}
          ...
          {!comment?.isPosting ? (
            <TouchableOpacity onPress={handleReply} style={{alignSelf: 'flex-start'}}>
              <CustomText variant="h9" style={styles.timestamp} fontFamily={FONTS.Medium}>
                Reply
              </CustomText>
            </TouchableOpacity>
        ...
        {!comment?.isPosting && (
          <TouchableOpacity
            ...
            <Icon
              name={
                (isReply && replyMeta?.isLiked) || (!isReply && commentMeta?.isLiked)
                  ? 'heart'
                  : 'heart-outline'
              }
              size={RFValue(12)}
            ...
            />
            <CustomText variant="h9" style={{color: Colors.lightText}} fontFamily={FONTS.Medium}>
              {isReply ? replyMeta.likesCount || '' : commentMeta.likesCount || ''}
            </CustomText>
          </TouchableOpacity>
        )}
      </Animated.View>
