...
const CommentInput: React.FC<commentInputProps> = ({
  ...
  const replaceMentionAtCursor = (
    ...
    if (index !== -1 && index < cursorPos) {
      return text.slice(0, index) + `@${newWord} ` + text.slice(index + replaceWord.length + 1);
    }
    return text;
  };
          ...
          <View style={styles.flexRow}>
            ...
            {comment ? (
              <TouchableOpacity onPress={() => handleSend(null)}>
                <Icon name={comment ? 'send' : 'emoji-emotions'} size={24} color={Colors.text} />
              </TouchableOpacity>
            ) : (
              <TouchableOpacity
                onPress={async () => {
                ...
                <Image source={GIFIcon} tintColor={Colors.text} style={styles.gifIcon} />
              </TouchableOpacity>
            )}
          </View>
        </View>
